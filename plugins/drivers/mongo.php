	...
	if (class_exists('MongoDB\Driver\Manager')) {
		class Db extends SqlDb {
			...
			function attach($server, $username, $password): string {
				...
				$this->_db_name = idx($options, "db", "");
				return $this->attachWithRetries($server, $options);
			}
			...
			function executeDbCommand($db, $command, bool $throwErrors = false) {
				try {
			...
			private function attachWithRetries($server, array $options): string {
				$lastError = '';
				foreach (array("mongodb+srv://", "mongodb://") as $uriPrefix) {
					try {
						$this->_link = new \MongoDB\Driver\Manager($uriPrefix . $server, $options);
						$this->executeDbCommand('admin', array('ping' => 1), true);
						return '';
					} catch (\Exception $e) {
				...
				if (empty($options["authSource"]) && !empty($options["db"])) {
	...
	function alter_table($table, $name, $fields, $foreign, $comment, $engine, $collation, $auto_increment, $partitioning) {
		if ($table == "") {
			if (!is_null(connection()->_db)) {
				...
				connection()->executeCommand(array('create' => $name));
				if (connection()->error) {
					return false;
				}
			}
	...
	function drop_tables($tables) {
		foreach ($tables as $table) {
			if (!is_null(connection()->_db)) {
				...
				connection()->executeCommand(array('drop' => $table));
				if (connection()->error) {
					return false;
				}
			}
	...
	function truncate_tables($tables) {
		foreach ($tables as $table) {
			if (!is_null(connection()->_db)) {
				...
				connection()->executeCommand(array('remove' => $table));
				if (connection()->error) {
					return false;
				}
			}
